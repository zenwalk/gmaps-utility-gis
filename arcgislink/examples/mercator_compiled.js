...
Q.prototype.i = function(a) {
  ...
  return[(this.j + (b - (1 + 2 * d + c) * Math.pow(b, 3) / 6 + (5 - 2 * c + 28 * d - 3 * c * c + 8 * this.s + 24 * d * d) * Math.pow(b, 5) / 120) / Math.cos(a)) / i, e / i]
};
...
function ba(a, b) {
  ...
  if(b instanceof M) {
    ...
    if(e !== "") {
      ...
      d.r = parseFloat(u(c, '"Central_Meridian",', "]"));
