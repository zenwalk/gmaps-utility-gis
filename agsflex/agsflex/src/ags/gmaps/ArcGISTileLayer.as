  ...
  public class ArcGISTileLayer extends TileLayerBase implements IEventDispatcher {
    ...
    public function ArcGISTileLayer(service:*, opt_layerOpts:ArcGISTileLayerOptions=undefined) {
      ...
      if (opt_layerOpts.hosts) {
        var pro:String=Util.extractString(this.mapService_.url, '', '://');
        var host:String=Util.extractString(this.mapService_.url, '://', '/');
        var path:String=Util.extractString(this.mapService_.url, pro + '://' + host, '');
        this.urlTemplate_=pro + '://' + opt_layerOpts.hosts + path;
        this.numOfHosts_=parseInt(Util.extractString(opt_layerOpts.hosts, '[', ']'), 10);
      }
