...
dojo.declare("agsjs.layers.GoogleMapsLayer", esri.layers.Layer, {
  ...
  _setMap: function(a, b) {
    ...
    dojo.style(d, c);
    this._element = d;
    var e = dojo.create("div", {}, d);
    dojo.style(e, c);
    dojo.style(e, "width", (a.width || b.offsetWidth) + "px");
    dojo.style(e, "height", (a.height || b.offsetHeight) + "px");
    this._gmapDiv = e;
    var f = dojo.create("div", {}, a.id);
    f.id = "gmaps_top_" + e.id;
    dojo.style(f, c);
    this._topDiv = f;
    f = dojo.create("div", {}, a.id);
    f.id = "gmaps_controls_" + e.id;
    dojo.style(f, dojo.mixin(c, {
      top: "5px",
      left: "5px"
    }));
    this._controlDiv = f;
    ...
    (this.visible = this.visible === undefined ? true : this.visible) && this._initGMap();
    return d
  },
  _unsetMap: function() {
    dojo.forEach(this._connects, dojo.disconnect, dojo);
    this._streetView && this._streetView.setVisible(false);
    if (google && google.maps &&
    google.maps.event) {
      this._gmapTypeChangeHandle && google.maps.event.removeListener(this._gmapTypeChangeHandle);
      this._svVisibleHandle && google.maps.event.removeListener(this._svVisibleHandle)
    }
    this._element && this._element.parentNode.removeChild(this._element);
    dojo.destroy(this._element);
    this._controlDiv && this._controlDiv.parentNode.removeChild(this._controlDiv);
    dojo.destroy(this._controlDiv);
    this._topDiv && this._topDiv.parentNode.removeChild(this._topDiv);
    dojo.destroy(this._topDiv)
  },
  _initGMap: function() {
    if (window.google &&
    google.maps) {
      var a = this._map.extent, b = this._mapOptions.center || this._esriPointToLatLng(a.getCenter()), c = this._map.getLevel();
      b = dojo.mixin({
        center: b,
        zoom: c > -1 ? c : 1,
        panControl: false,
      ...
      b.mapTypeId = b.mapTypeId ? this._getGMapTypeId(b.mapTypeId) : google.maps.MapTypeId.ROADMAP;
      b = new google.maps.Map(this._gmapDiv, b);
      c < 0 && dojo.connect(this._map, "onLoad", dojo.hitch(this, function() {
        this._setExtent(a)
      }));
      this._gmap = b;
      this._setExtent(a);
      ...
      if (d.toLowerCase().indexOf("http") == -1) 
        d = "http:";
      d = d + "//maps.googleapis.com/maps/api/js?callback=agsjs.onGMapsApiLoad";
      for (b in this._apiOptions) 
        if (this._apiOptions.hasOwnProperty(b)) 
          d += "&" + b + "=" + this._apiOptions[b];
      c.src = d;
      document.getElementsByTagName("head").length > 0 ? document.getElementsByTagName("head")[0].appendChild(c) : document.body.appendChild(c)
    }
  },
  setOpacity: function(a) {
    if (this._gmapDiv) {
      a = Math.min(Math.max(a, 0), 1);
      var b = this._gmapDiv.style;
      if (typeof b.opacity !== "undefined") 
        b.opacity = a;
      else if (typeof b.filters !== "undefined") 
        b.filters.alpha.opacity = Math.floor(100 * a);
      else if (typeof b.filter !== "undefined") 
  ...
  setMapTypeId: function(a) {
    ...
    } else this._mapOptions.mapTypeId = a
  },
  ...
  _visibilityChangeHandler: function(a) {
    if (a) {
      ...
      if (this._gmap) {
        ...
        this._extentChangeHandle = this._extentChangeHandle ||
        dojo.connect(this._map, "onExtentChange", this, this._extentChangeHandler);
        this._setExtent(this._map.extent)
      } else this._initGMap()
    } else if (this._gmapDiv) {
      esri.hide(this._gmapDiv);
      esri.hide(this._controlDiv);
      this.visible = false;
      this._gmap && this._map.setExtent(this._latLngBoundsToEsriExtent(this._gmap.getBounds()));
      this._streetView && this._streetView.setVisible(false);
      if (this._panHandle) {
  ...
  _panHandler: function(a) {
    this._gmap.getTilt() == 0 && this._gmap.setCenter(this._esriPointToLatLng(a.getCenter()))
  },
  ...
  _checkZoomLevel: function() {
    var a = this._gmap.getMapTypeId(), b = this._gmap.mapTypes, c = null;
    for (var d in b) 
    ...
    if (c != null) {
      ...
      c !== undefined && b > c && this._map.setLevel(c);
      a != undefined && b < a && this._map.setLevel(a)
    }
  },
  _setExtent: function(a) {
    var b = this._esriPointToLatLng(a.getCenter());
    this._map.getLevel();
    this._gmap.fitBounds(this._esriExtentToLatLngBounds(a.expand(0.5)));
    this._gmap.setCenter(b);
    this._checkZoomLevel()
  },
  _moveControls: function() {
    if (this._mvHandle) 
      if (this._gmap) {
        if (!this._svMoved) 
          if (this._streetView = this._gmap.getStreetView()) {
            var a = dojo.query('.gmnoprint img[src*="cb_scout_sprite"]', this._gmapDiv);
            if (a.length > 0) {
              dojo.forEach(a, function(b) {
                dojo.place(b.parentNode.parentNode, this._controlDiv)
              }, this);
              this._svMoved = true;
        ...
        if (!this._rotateMoved) {
          a = dojo.query('.gmnoprint img[src*="rotate"]', this._gmapDiv);
          if (a.length > 0) {
            dojo.forEach(a, function(b) {
              dojo.place(b.parentNode.parentNode, this._controlDiv);
              dojo.style(b, "position", "absolute");
              dojo.style(b, "left", "20px")
            }, this);
  ...
  _toggleEsriControl: function(a) {
    if (a) {
      ...
      this._isZoomSliderDefault && this._map.showZoomSlider();
      this._map.enableMapNavigation()
    }
  },
  onStreetViewVisibilityChange: function() {
  },
  ...
  _latLngBoundsToEsriExtent: function(a) {
    a = new esri.geometry.Extent(a.getSouthWest().lng(), a.getSouthWest().lat(), a.getNorthEast().lng(), a.getNorthEast().lat());
...
dojo.extend(esri.dijit.BasemapGallery, {
  ...
  postMixInProperties: function() {
    ...
    this._original_postMixInProperties && this._original_postMixInProperties()
  },
  startup: function() {
  ...
  _onLoadExt: function() {
    this._onLoadListenerExt && dojo.disconnect(this._onLoadListenerExt);
    this.toggleReference &&
    dojo.forEach(this.basemaps, function(a) {
      var b = a.getLayers();
      b.length ? this._processReferenceLayersExt(a) : b.then(dojo.hitch(this, this._processReferenceLayersExt, a))
    }, this)
  },
  _processReferenceLayersExt: function(a) {
    var b = a.getLayers(), c = false, d = true;
    ...
    if (c && this.toggleReference) {
      a.title += '<input type="checkbox"  disabled ' + (d ? "checked" : "") + "/>";
      a._hasReference = true
    } else a._hasReference = false;
    var e = 0;
    dojo.forEach(this.basemaps, function(f) {
      if (f._hasReference == undefined) 
        e -= this.basemaps.length;
      f._hasReference && e++
    }, this);
    e >= 0 && e > 0 && this._startupCalled && this.startup()
  },
  _onSelectionChangeExt: function() {
    var a = this.getSelected(), b;
    if (a) {
      if (this._googleLayers.length > 0) {
        dojo.forEach(this._googleLayers, function(c) {
          this.map.removeLayer(c)
        }, this);
        this._googleLayers.length = 0
      }
      dojo.query("input", this.domNode).forEach(function(c) {
        c.disabled = true
      });
      a = a.getLayers();
      dojo.forEach(a, function(c) {
        if (c.type && c.type.indexOf("GoogleMaps") > -1) {
          var d = agsjs.layers.GoogleMapsLayer.MAP_TYPE_ROADMAP;
          if (c.type == "GoogleMapsSatellite") 
            d = agsjs.layers.GoogleMapsLayer.MAP_TYPE_SATELLITE;
          else if (c.type == "GoogleMapsHybrid") 
            d = agsjs.layers.GoogleMapsLayer.MAP_TYPE_HYBRID;
          this.google = this.google || {};
          this.google.mapOptions = this.google.mapOptions || {};
          this.google.mapOptions.mapTypeId = d;
    ...
    if (!this._checkSelectedNode() && this._startupCalled) {
      this.startup();
      this._checkSelectedNode()
    }
  },
  _checkSelectedNode: function() {
    var a = false, b = this.getSelected().getLayers();
    dojo.query(".esriBasemapGallerySelectedNode", this.domNode).forEach(function(c) {
      a = true;
      dojo.query("input", c).forEach(function(d) {
        d.disabled = false;
        dojo.forEach(b, function(e) {
          if (e.isReference) 
            e.visibility = d.checked
        }, this)
      }, this)
    }, this);
    return a
  },
  _onGalleryClickExt: function(a) {
    a = a.target;
    if (a.tagName.toLowerCase() == "input") 
      this._setReferenceVis(a.checked);
    else dojo.hasClass(a.parentNode, "esriBasemapGalleryLabelContainer") && a.parentNode.parentNode.firstChild.click()
  },
  _setReferenceVis: function(a) {
    dojo.forEach(this.map.layerIds, function(b) {
      b = this.map.getLayer(b);
      if (b._basemapGalleryLayerType == "reference") 
        a ? b.show() : b.hide()
    }, this)
  },
