...
dojo.addOnLoad(function() {
   dojo.declare("agsjs.layers.ExtrudedFeatureLayer", esri.layers.Layer, {
     _osmb: null,
     _canvas: null,
     _tileInfo: null,
   ...
   * @property {int} [mode] optional. agsjs.layers.ExtrudedFeatureLayer.MODE_ONDEMAND | MODE_SNAPSHOT. default=ON_DEMAND
   * @property {esri.tasks.Query} [query] optional. query set on the feature layer for retrieving data.
   * @property {Object} [style] object with color, roofColor (#ffcc00' 'rgb(255,200,200)' 'rgba(255,200,200,0.9)), strokeRoofs (boolean)
   */
    /** 
   * Create an ExtrudedFeatureLayer
   * @name ExtrudedFeatureLayer
   * @constructor
   * @class This class is a ExtrudedFeatureLayer, such as a polygon feature layer with a height attribute.
   * @param {string||FeatureCollection} url
   * @param {ExtrudedFeatureLayerOptions} opts
   */
    constructor: function(url, opts) {
      ...
      this._mode = opts.mode || agsjs.layers.ExtrudedFeatureLayer.MODE_ONDEMAND;
      ...
      if (dojo.isObject(url) && url.featureSet) {
        this._mode = agsjs.layers.ExtrudedFeatureLayer.MODE_SNAPSHOT;
   ...
   **********************/
    ...
    _loadData: function() {
      if (this._mode == agsjs.layers.ExtrudedFeatureLayer.MODE_SNAPSHOT) {
        if (this._oids) {
          return;
        } else {
   ...
   dojo.mixin(agsjs.layers.ExtrudedFeatureLayer, {
     MODE_ONDEMAND: 0,
     MODE_SNAPSHOT: 1
   });
});
