 ...
 * @param {String} end
 */
gmaps.ags.extractString_ = function (full, start, end) {
 ...
 * @param {Object} o
 */
gmaps.ags.isString_ = function(o) {
 ...
 * @return {Object}
 */
gmaps.ags.augmentObject_ = function(src, dest, force) {
  if (src && dest) {
    var p;
    for (p in src) {
      if (force || !(p in dest)) {
 ...
 * @param {Object} args
 */
gmaps.ags.triggerEvent_ = function(src, evtName, args) {
  G.event.trigger.apply(src, arguments);
};
 ...
 * @param {Object} json
 */
gmaps.ags.handleErr_ = function(errback, json) {
 ...
 * @param {Date} endTime
 */
gmaps.ags.formatTimeString_ = function(time, endTime) {
  var ret = '';
  if (time) {
 ...
 * @param {Number} 0-1
 */
gmaps.ags.setNodeOpacity_ = function(node, op) {
 ...
 * @param {Object} defs
 */
gmaps.ags.getLayerDefsString_ = function (defs) {
  var strDefs = '';
  for (var x in defs) {
...
gmaps.ags.getXmlHttp_ = function() {
...
gmaps.ags.getGeometryType_ = function(obj) {
  var o = obj;
  if (gmaps.ags.isArray_(obj) && obj.length > 0) {
    o = obj[0];
  }
  if (o instanceof G.LatLng || o instanceof G.Marker) {
    if (gmaps.ags.isArray_(obj) && obj.length > 1) {
      return gmaps.ags.GeometryType.MULTIPOINT;
    } else {
      return gmaps.ags.GeometryType.POINT;
    }
  } else if (o instanceof G.Polyline) {
    return gmaps.ags.GeometryType.POLYLINE;
  } else if (o instanceof G.Polygon) {
    return gmaps.ags.GeometryType.POLYGON;
  } else if (o instanceof G.LatLngBounds) {
    return gmaps.ags.GeometryType.ENVELOPE;
 ...
 * @return {Boolean}
 */
gmaps.ags.isOverlay_ = function(obj) {
  var o = obj;
  if (gmaps.ags.isArray_(obj) && obj.length > 0) {
    o = obj[0];
  }
  if (gmaps.ags.isArray_(o) && o.length > 0) {
    o = o[0];
  }
  if (o instanceof G.LatLng || o instanceof G.Marker ||
...
gmaps.ags.formatSRParam_ = function (sr) {
  if (!sr) {
    return null;
  }
 ...
 */
gmaps.ags.fromLatLngsToJSON_ =function (pts, close) {
  var arr = [];
  var latlng;
 ...
 */
gmaps.ags.fromOverlaysToJSON_ = function (geom) {
  var gtype = gmaps.ags.getGeometryType_(geom);
  var g, gs, i, pts;
  var json = '{';
  switch (gtype) {
  case gmaps.ags.GeometryType.POINT:
    g = gmaps.ags.isArray_(geom) ? geom[0] : geom;
    if (g instanceof G.Marker) {
      g = g.getPosition();
    }
    json += 'x:' + g.lng() + ',y:' + g.lat();
    break;
  case gmaps.ags.GeometryType.MULTIPOINT:
    pts = [];
    ...
    break;
  case gmaps.ags.GeometryType.POLYLINE:
    // V3 does not support multiple paths yet
    pts = [];
    gs = gmaps.ags.isArray_(geom) ? geom : [geom];
    for (i = 0; i < gs.length; i++) {
      pts.push('[' + gmaps.ags.fromLatLngsToJSON_(gs[i].getPath()) + ']');
    }
    json += 'paths:[' + pts.join(',') + ']';
    break;
  case gmaps.ags.GeometryType.POLYGON:
    pts = [];
    g = gmaps.ags.isArray_(geom) ? geom[0] : geom;
    var paths = g.getPaths();
    for (i = 0; i < paths.getLength(); i++) {
      pts.push('[' + gmaps.ags.fromLatLngsToJSON_(paths.getAt(i), true) + ']');
    }
    json += 'rings:[' + pts.join(',') + ']';
    
    break;
  case gmaps.ags.GeometryType.ENVELOPE:
    g = gmaps.ags.isArray_(geom) ? geom[0] : geom;
 ...
 * @param {Object} geom
 */
gmaps.ags.fromGeometryToJSON_ = function (geom) {
 ...
 */
gmaps.ags.fromEnvelopeToLatLngBounds_ = function (extent) {
 ...
 */
gmaps.ags.fromJSONToOverlays_ = function (geom, opts) {
  var ovs = null;
  var ov;
  ...
  if (geom) {
    ovs = [];
    if (geom.x) {
      ov = new G.Marker(gmaps.ags.augmentObject_(opts.markerOptions || opts, {
      ...
      for (i = 0, ic = parts.length; i < ic; i++) {
        ...
        if (geom.points) {
          ...
          if (geom.paths) {
            ov = new G.Polyline(gmaps.ags.augmentObject_(opts.polylineOptions || opts, {
              'path': latlngs
            }));
            ovs.push(ov);
          } else if (geom.rings) {
      ...
      if (geom.rings) {
        ov = new G.Polygon(gmaps.ags.augmentObject_(opts.polygonOptions || opts, {
          'paths': rings
        }));
        ovs.push(ov);
      }
...
gmaps.ags.parseFeatures_ = function (features, ovOpts) {
  if (features) {
    var i, I, f;
    for (i = 0, I = features.length; i < I; i++) {
      f = features[i];
      if (f.geometry) {
        f.geometry = gmaps.ags.fromJSONToOverlays_(f.geometry, ovOpts);
      }
    }
  }
};
 ...
 * @param {Object} o
 */
gmaps.ags.formatRequestString_ = function(o) {
  var ret;
  if (typeof o === 'object') {
    if (gmaps.ags.isArray_(o)) {
      ret = [];
      for (var i = 0, I = o.length; i < I; i++) {
        ret.push(gmaps.ags.formatRequestString_(o[i]));
      }
      return '[' + ret.join(',') + ']';
    } else if (gmaps.ags.isOverlay_(o)) {
      return gmaps.ags.fromOverlaysToJSON_(o);
    } else if (o.toJSON) {
      return o.toJSON();
    } else {
      ret = '';
      for (var x in o) {
        if (o.hasOwnProperty(x)) {
          ...
          ret += x + ':' + gmaps.ags.formatRequestString_(o[x]);
        }
      }
      return '{' + ret + '}';
    }
...
gmaps.ags.fromLatLngsToFeatureSet_ = function(latlngs) {
  var i, I, latlng;
  var features = [];
...
gmaps.ags.prepareGeometryParams_ = function (p) {
  ...
  if (p.geometries && p.geometries.length > 0) {
    g = p.geometries[0];
    isOv = gmaps.ags.isOverlay_(g);
    for (var i = 0, c = p.geometries.length; i < c; i++) {
      if (isOv) {
        json.push(gmaps.ags.fromOverlaysToJSON_(p.geometries[i]));
      } else {
        json.push(gmaps.ags.fromGeometryToJSON_(p.geometries[i]));
      }
    }
  }
  if (!p.geometryType) {
    p.geometryType = gmaps.ags.getGeometryType_(g);
  }
  if (isOv) {
    params.inSR = WGS84.wkid;
  } else if (p.inSpatialReference) {
    params.inSR = gmaps.ags.formatSRParam_(p.inSpatialReference);
  }
  if (p.outSpatialReference) {
    params.outSR = gmaps.ags.formatSRParam_(p.outSpatialReference);
  }
  params.geometries = '{geometryType:"' + p.geometryType + '", geometries:[' + json.join(',') + ']}';
  return params;
};

gmaps.ags.log_ = function (msg) {
  if (window.console) {
 ...
 * @param {Object} params
 */
gmaps.ags.formatParams_ = function (/** @type {JSONParams} */params) {
  var query = '';
  if (params) {
    ...
    for (var x in params) {
      if (params.hasOwnProperty(x) && params[x] !== null && params[x] !== undefined) { // wont sent undefined.
        ...
        var val = gmaps.ags.formatRequestString_(params[x]);
 ...
 */
gmaps.ags.Util.getJSON = function(url, params, callbackName, callbackFn) {
  var sid = 'ags_jsonp_' + (jsonpID_++) + '_' + Math.floor(Math.random() * 1000000);
  var script = null;
  params = params || {};
  params[callbackName || 'callback'] = sid + ' && ' + sid;
  var query = gmaps.ags.formatParams_(params);
  ...
  var jsonpcallback = function() {
     ...
     * @event
     */
    //gmaps.ags.triggerEvent_(me, 'jsonpend', sid);
  };
  window[sid] = jsonpcallback;
  
  if ((query + url).length < 2000 && !gmaps.ags.Config.alwaysUseProxy) {
    ...
    if (gmaps.ags.Config.alwaysUseProxy) {
      useProxy = true;
    }
    if (useProxy && !gmaps.ags.Config.proxyUrl) {
      throw new Error('No proxyUrl property in Config is defined');
    }
    var xmlhttp = gmaps.ags.getXmlHttp_();
    ...
    xmlhttp.open('POST', useProxy ? gmaps.ags.Config.proxyUrl + '?' + url : url, true);
 ...
 */
gmaps.ags.Util.addToMap = function(map, overlays) {
  if (gmaps.ags.isArray_(overlays)) {
    var ov;
    for (var i = 0, I = overlays.length; i < I; i++) {
      ov = overlays[i];
      if (gmaps.ags.isArray_(ov)) {
        gmaps.ags.Util.addToMap(map, ov);
      } else if (gmaps.ags.isOverlay_(ov)) {
        ov.setMap(map);
      }
    }
  }
 ...
 */
gmaps.ags.Util.removeFromMap = function(overlays, clearArray) {
  gmaps.ags.Util.addToMap(null, overlays);
  if (clearArray) {
    overlays.length = 0;
  }
};
 ...
 * @return {Array.number}
 */
gmaps.ags.SpatialReference.prototype.forward = function(lnglat) {
  return lnglat;
};
/**
 ...
 * @return {Array.number}
 */
gmaps.ags.SpatialReference.prototype.inverse = function(coords) {
  return coords;
};
/**
 ...
 */
gmaps.ags.SpatialReference.prototype.getCircum = function() {
  return 360;
};
/**
 * To JSON String
 * @return String
 */
gmaps.ags.SpatialReference.prototype.toJSON = function() {
 ...
 * @param {Object} params
 */
gmaps.ags.Geographic = function(params) {
  params = params || {};
  gmaps.ags.SpatialReference.call(this, params);
};
gmaps.ags.Geographic.prototype = new gmaps.ags.SpatialReference();
 ...
 * @param {Object} params
 */
gmaps.ags.LambertConformalConic = function(params) {
...
gmaps.ags.LambertConformalConic.prototype = new gmaps.ags.SpatialReference();
/**
 * calc_m_
 * @param {number} phi
 * @param {number} es e square
 */
gmaps.ags.LambertConformalConic.prototype.calc_m_ = function(phi, es) {
 ...
 * @param {Object} e
 */
gmaps.ags.LambertConformalConic.prototype.calc_t_ = function(phi, e) {
 ...
 * @param {Object} n
 */
gmaps.ags.LambertConformalConic.prototype.calc_rho_ = function(a, F, t, n) {
 ...
 * @param {Object} phi
 */
gmaps.ags.LambertConformalConic.prototype.calc_phi_ = function(t, e, phi) {
  var esp = e * Math.sin(phi);
 ...
 * @param {Object} init
 */
gmaps.ags.LambertConformalConic.prototype.solve_phi_ = function(t_i, e, init) {
  // iteration
  var i = 0;
  var phi = init;
 ...
 * @return {Array.number}
 */
gmaps.ags.LambertConformalConic.prototype.forward = function(lnglat) {
 ...
 * @return {Array.number}
 */
gmaps.ags.LambertConformalConic.prototype.inverse = function(coords) {
 ...
 *  @return {number}
 */
gmaps.ags.LambertConformalConic.prototype.getCircum = function() {
 ...
 * @extends SpatialReference
 */
gmaps.ags.TransverseMercator = function(params) {
  params = params || {};
  gmaps.ags.SpatialReference.call(this, params);
...
gmaps.ags.TransverseMercator.prototype = new gmaps.ags.SpatialReference();
/**
 * calc_m_
 * @param {Object} phi
 * @param {Object} a
 ...
 * @param {Object} ep6
 */
gmaps.ags.TransverseMercator.prototype.calc_m_ = function(phi, a, es, ep4, ep6) {
 ...
 * @return {Array.number}
 */
gmaps.ags.TransverseMercator.prototype.forward = function(lnglat) {
  var phi = lnglat[1] * RAD_DEG;// (Math.PI / 180);
 ...
 * @return {Array.number}
 */
gmaps.ags.TransverseMercator.prototype.inverse = function(coords) {
  var E = coords[0];
  var N = coords[1];
 ...
 * @return number
 */
gmaps.ags.TransverseMercator.prototype.getCircum = function() {
  return Math.PI * 2 * this.a_;
};
 ...
 * @extends SpatialReference
 */
gmaps.ags.SphereMercator = function(params) {
  /*  =========== parameters  =  ===================== */
  params = params || {};
  gmaps.ags.SpatialReference.call(this, params);
...
gmaps.ags.SphereMercator.prototype = new gmaps.ags.SpatialReference();
 ...
 * @return {Array.number}
 */
gmaps.ags.SphereMercator.prototype.forward = function(lnglat) {
 ...
 * @return {Array.number}
 */
gmaps.ags.SphereMercator.prototype.inverse = function(coords) {
  var E = coords[0];
  var N = coords[1];
 ...
 * @param {Object} params
 */
gmaps.ags.Albers = function(params) {
...
gmaps.ags.Albers.prototype = new gmaps.ags.SpatialReference();
/**
 * calc_m_
 * @param {number} phi
 * @param {number} es e square
 */
gmaps.ags.Albers.prototype.calc_m_ = function(phi, es) {
  var sinphi = Math.sin(phi);
  return Math.cos(phi) / Math.sqrt(1 - es * sinphi * sinphi);
};
 ...
 * @param {Object} e
 */
gmaps.ags.Albers.prototype.calc_q_ = function(phi, e) {
  var esp = e * Math.sin(phi);
  return (1 - e * e) * (Math.sin(phi) / (1 - esp * esp) - (1 / (2 * e)) * Math.log((1 - esp) / (1 + esp)));
};

gmaps.ags.Albers.prototype.calc_rho_ = function(a, C, n, q) {
  return a * Math.sqrt(C - n * q) / n;
};

gmaps.ags.Albers.prototype.calc_phi_ = function(q, e, phi) {
  var esp = e * Math.sin(phi);
  return phi + (1 - esp * esp) * (1 - esp * esp) / (2 * Math.cos(phi)) * (q / (1 - e * e) - Math.sin(phi) / (1 - esp * esp) + Math.log((1 - esp) / (1 + esp)) / (2 * e));
};

gmaps.ags.Albers.prototype.solve_phi_ = function(q, e, init) {
  // iteration
  var i = 0;
  var phi = init;
 ...
 * @return {Array.number}
 */
gmaps.ags.Albers.prototype.forward = function(lnglat) {
  var phi = lnglat[1] * RAD_DEG;
  var lamda = lnglat[0] * RAD_DEG;
 ...
 * @return {Array.number}
 */
gmaps.ags.Albers.prototype.inverse = function(coords) {
  var E = coords[0] - this.FE_;
  var N = coords[1] - this.FN_;
 ...
 *  @return number
 */
gmaps.ags.Albers.prototype.getCircum = function() {
  return Math.PI * 2 * this.a_;
};
/**
 * See {@link SpatialReference}
...
WGS84 = new gmaps.ags.Geographic({
  wkid: 4326
});
NAD83 = new gmaps.ags.Geographic({
  wkid: 4269
});
WEB_MERCATOR = new gmaps.ags.SphereMercator({
  wkid: 102113,
  semi_major: 6378137.0,
  central_meridian: 0,
  unit: 1
});
WEB_MERCATOR_AUX = new gmaps.ags.SphereMercator({
  wkid: 102100,
  semi_major: 6378137.0,
  central_meridian: 0,
  unit: 1
...
gmaps.ags.SpatialReference.WGS84 = WGS84;
gmaps.ags.SpatialReference.NAD83 = NAD83;
gmaps.ags.SpatialReference.WEB_MERCATOR = WEB_MERCATOR;
gmaps.ags.SpatialReference.WEB_MERCATOR_AUX = WEB_MERCATOR_AUX;
 ...
 */
gmaps.ags.SpatialReference.register = function(wkidt, wktOrSR) {
  ...
  if (wktOrSR instanceof gmaps.ags.SpatialReference) {
    ...
    var prj = gmaps.ags.extractString_(wkt, "PROJECTION[\"", "\"]");
    var spheroid = gmaps.ags.extractString_(wkt, "SPHEROID[", "]").split(",");
    if (prj !== "") {
      params.unit = parseFloat(gmaps.ags.extractString_(gmaps.ags.extractString_(wkt, "PROJECTION", ""), "UNIT[", "]").split(",")[1]);
      params.semi_major = parseFloat(spheroid[1]);
      params.inverse_flattening = parseFloat(spheroid[2]);
      params.latitude_of_origin = parseFloat(gmaps.ags.extractString_(wkt, "\"Latitude_Of_Origin\",", "]"));
      params.central_meridian = parseFloat(gmaps.ags.extractString_(wkt, "\"Central_Meridian\",", "]"));
      params.false_easting = parseFloat(gmaps.ags.extractString_(wkt, "\"False_Easting\",", "]"));
      params.false_northing = parseFloat(gmaps.ags.extractString_(wkt, "\"False_Northing\",", "]"));
    }
    switch (prj) {
    case "":
 ...
 */
gmaps.ags.Catalog = function(url) {
  this.url = url;
  var me = this;
  getJSON_(url, {}, '', function(json) {
    gmaps.ags.augmentObject_(json, me);
    /**
     * This event is fired when the catalog info is loaded.
     * @name Catalog#load
     * @event
     */
    gmaps.ags.triggerEvent_(me, 'load');
  });
};
/**
 * @name Field
 ...
 */
gmaps.ags.Layer.prototype.load = function() {
  var me = this;
  if (this.loaded_) {
    return;
  }
  gmaps.ags.Util.getJSON(this.url, {}, '', function(json) {
    gmaps.ags.augmentObject_(json, me);
    me.loaded_ = true;
    /**
 ...
 * @return {Boolean}
 */
gmaps.ags.Layer.prototype.isInScale = function(scale) {
...
gmaps.ags.Layer.prototype.query = function(/** @type {JSONRequest} */p, callback, errback) {
  ...
  if (p.outFields && !gmaps.ags.isArray_(p.outFields)) {
  ...
  if (p.time) {
    params.time = gmaps.ags.formatTimeString_(p.time, p.endTime);
  }
  params.outSR = 4326;
  ...
  gmaps.ags.Util.getJSON(this.url + '/query', params, '', function(json) {
    gmaps.ags.parseFeatures_(json.features, p.overlayOptions);
    callback(json, json.error);
    gmaps.ags.handleErr_(errback, json);
  });
};
/**
 ...
 * @param {Function} errback
 */
gmaps.ags.Layer.prototype.queryRelatedRecords = function(qparams, callback, errback) {
  if (!qparams) {
    return;
  }
 ...
 */
gmaps.ags.MapService = function(url, opts) {
  this.url = url;
  this.loaded_ = false;
  var tks = url.split("/");
 ...
 * Load serviceInfo
 */
gmaps.ags.MapService.prototype.load = function() {
  var me = this;
  gmaps.ags.Util.getJSON(this.url, {}, '', function(json) {
    me.init_(json);
  });
 ...
 * @param {Object} json
 */
gmaps.ags.MapService.prototype.init_ = function(/** @type MapServiceResponse */json) {
  var me = this;
  ...
  if (json.tables !== undefined) {
    // v10.0 +
    gmaps.ags.Util.getJSON(this.url + '/layers', {}, '', function(json2) {
 ...
 * @param {Object} json2
 */
gmaps.ags.MapService.prototype.initLayers_ = function(json2) {
  var layers = [];
  var tables = [];
  this.layers_ = layers;
  if (json2.tables) {
   ...
   * @event
   */
  gmaps.ags.triggerEvent_(this, "load");
};
/**
 ...
 * @return {Layer}
 */
gmaps.ags.MapService.prototype.getLayer = function(nameOrId) {
  ...
  if (layers) {
    for (var i = 0, c = layers.length; i < c; i++) {
      ...
      if (gmaps.ags.isString_(nameOrId) && layers[i].name.toLowerCase() === nameOrId.toLowerCase()) {
        return layers[i];
      }
    }
  }
 ...
 */
gmaps.ags.MapService.prototype.getLayerDefs_ = function() {
  var ret = {};
  if (this.layers_) {
 ...
 * @return {Boolean}
 */
gmaps.ags.MapService.prototype.hasLoaded = function() {
  return this.loaded_;
};
/**
 ...
 */
gmaps.ags.MapService.prototype.getVisibleLayerIds_ = function() {
  var ret = [];
 ...
 */
gmaps.ags.MapService.prototype.getInitialBounds = function() {
  if (this.initialExtent) {
    return gmaps.ags.fromEnvelopeToLatLngBounds_(this.initialExtent);
  }
  return null;
};
gmaps.ags.MapService.prototype.getLayers = function() {
  return this.layers_;
};

gmaps.ags.MapService.prototype.getTables = function() {
  return this.tables_;
};
...
gmaps.ags.MapService.prototype.exportMap = function(/** @type {JSONRequest} */p, callback, errback) {
  ...
  params.layerDefs = gmaps.ags.getLayerDefsString_(defs);
  ...
  if (params.f === 'image') {
    return this.url + '/export?' + gmaps.ags.formatParams_(params);
  } else {
    gmaps.ags.Util.getJSON(this.url + '/export', params, '', function(json) {
      if (json.extent) {
        json.bounds = gmaps.ags.fromEnvelopeToLatLngBounds_(json.extent);
        delete json.extent;
        callback(json);
      } else {
        gmaps.ags.handleErr_(errback, json.error);
      }
    });
  }
};
...
gmaps.ags.MapService.prototype.identify = function(/** @type {JSONRequest} */p, callback, errback) {
  ...
  if (p.layerDefs) {
    params.layerDefs = gmaps.ags.getLayerDefsString_(p.layerDefs);//TODO
  }
  params.maxAllowableOffset = p.maxAllowableOffset;
  params.returnGeometry = (p.returnGeometry === false ? false : true);
  
  gmaps.ags.Util.getJSON(this.url + '/identify', params, '', function(json) {
    // process results;
    var rets = null;
    var i, js, g;
    if (json.results) {
      ...
      for (i = 0; i < json.results.length; i++) {
        js = json.results[i];
        g = gmaps.ags.fromJSONToOverlays_(js.geometry, p.overlayOptions);
        js.feature = {
          'geometry': g,
 ...
 * @param {Function} errback
 */
gmaps.ags.MapService.prototype.find = function(opts, callback, errback) {
  if (!opts) {
    return;
  }
  ...
  if (opts.layerDefinitions) {
    params.layerDefs = gmaps.ags.getLayerDefsString_(opts.layerDefinitions);
    delete params.layerDefinitions;
  }
  params.sr = 4326;
  params.returnGeometry = (opts.returnGeometry === false ? false : true);
  gmaps.ags.Util.getJSON(this.url + '/find', params, '', function(json) {
    var rets = null;
    var i, js, g;
    if (json.results) {
      rets = [];
      for (i = 0; i < json.results.length; i++) {
        js = json.results[i];
        g = gmaps.ags.fromJSONToOverlays_(js.geometry, opts.overlayOptions);
        js.feature = {
          'geometry': g,
          'attributes': js.attributes
        };
 ...
 */
gmaps.ags.GeocodeService = function(url) {
  this.url = url;
  this.loaded = false;
  var me = this;
  gmaps.ags.Util.getJSON(url, {}, '', function(json) {
    me.init_(json);
  });
};
/**
 ...
 * @param {Object} json
 */
gmaps.ags.GeocodeService.prototype.init_ = function(json) {
  gmaps.ags.augmentObject_(json, this);
...
gmaps.ags.GeocodeService.prototype.findAddressCandidates = function(gparams, callback, errback) {
  ...
  if (gmaps.ags.isArray_(params.outFields)) {
    params.outFields = params.outFields.join(',');
  }
  params.outSR = 4326;
  var me = this;
  gmaps.ags.Util.getJSON(this.url + '/findAddressCandidates', params, '', function(json) {
    if (json.candidates) {
      var res, loc;
 ...
 * @param {Function} callback
 */
gmaps.ags.GeocodeService.prototype.geocode = function(params, callback) {
 ...
 * @param {Function} errback
 */
gmaps.ags.GeocodeService.prototype.reverseGeocode = function(params, callback, errback) {
  if (!gmaps.ags.isString_(params.location)) {
    params.location = gmaps.ags.fromOverlaysToJSON_(params.location);
  }
  params.outSR = 4326;
  var me = this;
  gmaps.ags.Util.getJSON(this.url + '/reverseGeocode', params, '', function(json) {
    if (json.location) {
 ...
 * Params:<li><code>url</code>: URL of service, syntax:<code>	http://{catalog_url}/{serviceName}/GeometryServer</code>
 ...
 * @param {String} url
 */
gmaps.ags.GeometryService = function(url) {
  this.url = url;
};
  ...
  gmaps.ags.Util.getJSON(this.url + '/project', params, "callback", function(json) {
    ...
    if (p.outSpatialReference === 4326 || p.outSpatialReference.wkid === 4326) {
      for (var i = 0, c = json.geometries.length; i < c; i++) {
        geom.push(gmaps.ags.fromJSONToOverlays_(json.geometries[i]));
      }
...
gmaps.ags.GeometryService.prototype.buffer = function(/** @type {JSONRequest} */p, callback, errback) {
  ...
  gmaps.ags.Util.getJSON(this.url + '/buffer', params, "callback", function(json) {
    var geom = [];
    if (json.geometries) {
      for (var i = 0, c = json.geometries.length; i < c; i++) {
        geom.push(gmaps.ags.fromJSONToOverlays_(json.geometries[i], p['overlayOptions']));
      }
    }
    json.geometries = geom;
    callback(json);
 ...
 */
gmaps.ags.GPService = function(url) {
  this.url = url;
  this.loaded = false;
  var me = this;
  gmaps.ags.Util.getJSON(url, {}, '', function(json) {
...
gmaps.ags.GPTask.prototype.execute = function(/** @type {JSONRequest} */p, callback, errback) {
  ...
  if (p.outSpatialReference) {
    params['env:outSR'] = gmaps.ags.formatSRParam_(p.outSpatialReference);
  } else {
    params['env:outSR'] = 4326;
  }
  if (p.processSpatialReference) {
    params['env:processSR'] = gmaps.ags.formatSRParam_(p.processSpatialReference);
  }
  gmaps.ags.Util.getJSON(this.url + '/execute', params, '', function(json) {
    if (json.results) {
      var res, f;
      for (var i = 0; i < json.results.length; i++) {
        ...
        if (res.dataType === 'GPFeatureRecordSetLayer') {
          for (var j = 0, J = res.value.features.length; j < J; j++) {
            ...
            if (f.geometry) {
              f.geometry = gmaps.ags.fromJSONToOverlays_(f.geometry, p.overlayOptions);
            }
          }
        }
      }
 ...
 */
gmaps.ags.NetworkService = function(url) {
  this.url = url;
  this.loaded = false;
  var me = this;
  gmaps.ags.Util.getJSON(url, {}, '', function(json) {
 ...
 * @param {String} url
 */
gmaps.ags.RouteTask = function(url) {
  this.url = url;
};
 ...
 * @param {Function} errback
 */
gmaps.ags.RouteTask.prototype.solve = function(opts, callback, errback) {
  if (!opts) {
    return;
  }
  ...
  if (gmaps.ags.isArray_(opts.barriers)) {
    if (opts.barriers.length > 0) {
      params.barriers = gmaps.ags.fromLatLngsToFeatureSet_(opts.barriers);
    } else {
  ...
  gmaps.ags.Util.getJSON(this.url + '/solve', params, '', function(json) {
    if (json.routes) {
      gmaps.ags.parseFeatures_(json.routes.features, opts.overlayOptions);
    }
    callback(json);
    gmaps.ags.handleErr_(errback, json);
  });
 ...
 */
gmaps.ags.Projection = function(tileInfo) {
  //if (!tileInfo) {
 ...
 * @return {Point} pixel
 */
gmaps.ags.Projection.prototype.fromLatLngToPoint = function(latlng, opt_point) {
 ...
 * @return {LatLng}
 */
gmaps.ags.Projection.prototype.fromPointToLatLng = function(pixel, opt_nowrap) {
  //TODO: handle nowrap
  if (pixel === null) {
    return null;
  }
 ...
 * @return {Number}
 */
gmaps.ags.Projection.prototype.getScale = function(zoom) {
...
gmaps.ags.Projection.WEB_MECATOR = new gmaps.ags.Projection();
/**
 ...
 */
gmaps.ags.TileLayer = function(service, opt_layerOpts) {
 ...
 * @return {String} url
 */
gmaps.ags.TileLayer.prototype.getTileUrl = function(tile, zoom) {
 ...
 * @param {Number} op (0-1)
 */
gmaps.ags.TileLayer.prototype.setOpacity = function(op) {
  this.opacity_ = op;
  var tiles = this.tiles_;
  for (var x in tiles) {
    if (tiles.hasOwnProperty(x)) {
      gmaps.ags.setNodeOpacity_(tiles[x], op);
    }
  }
};
/**
 ...
 * @return {Number}
 */
gmaps.ags.TileLayer.prototype.getOpacity = function() {
 ...
 * @return {MapService}
 */
gmaps.ags.TileLayer.prototype.getMapService = function() {
 ...
 */
gmaps.ags.MapType = function(tileLayers, opt_typeOpts) {
 ...
 * @return {Node}
 */
gmaps.ags.MapType.prototype.getTile = function(tileCoord, zoom, ownerDocument) {
...
// always need export because google API calls it.
gmaps.ags.MapType.prototype['getTile'] = gmaps.ags.MapType.prototype.getTile;
/**
 * Release tile and cleanup
 * @param {Node} node
 */
gmaps.ags.MapType.prototype.releaseTile = function(node) {
...
gmaps.ags.MapType.prototype['releaseTile'] = gmaps.ags.MapType.prototype.releaseTile;

/**
 * Set Opactity
 * @param {Number} op
 */
gmaps.ags.MapType.prototype.setOpacity = function(op) {
  this.opacity_ = op;
  var tiles = this.tiles_;
  for (var x in tiles) {
    if (tiles.hasOwnProperty(x)) {
      ...
      for (var i = 0; i < nodes.length; i++) {
        gmaps.ags.setNodeOpacity_(nodes[i], op);
      }
    }
  }
};

gmaps.ags.MapType.prototype.getOpacity = function() {
  return this.opacity_;
};
/**
 ...
 */
gmaps.ags.MapType.prototype.getTileLayers = function() {
...
gmaps.ags.MapOverlay.prototype = new G.OverlayView();
/**
 * Handler when overlay is added. Interface method.
 * This will be called after setMap(map) is called.
 */
gmaps.ags.MapOverlay.prototype.onAdd = function() {
  ...
  if (this.opacity_) {
    gmaps.ags.setNodeOpacity_(div, this.opacity_);
  }
  var me = this;
...
gmaps.ags.MapOverlay.prototype['onAdd'] = gmaps.ags.MapOverlay.prototype.onAdd;

/** remove overlay
 */
gmaps.ags.MapOverlay.prototype.onRemove = function() {
  G.event.removeListener(this.boundsChangedListener_);
  this.div_.parentNode.removeChild(this.div_);
  this.div_ = null;
};
gmaps.ags.MapOverlay.prototype['onRemove'] = gmaps.ags.MapOverlay.prototype.onRemove;
...
gmaps.ags.MapOverlay.prototype['draw'] = gmaps.ags.MapOverlay.prototype.draw;

/**
 * Gets Image Opacity. return <code>opacity</code> between 0-1.
 * @return {Number} opacity
 */
gmaps.ags.MapOverlay.prototype.getOpacity = function() {
  return this.opacity_;
};
/**
 ...
 */
gmaps.ags.MapOverlay.prototype.setOpacity = function(opacity) {
 ...
 */
gmaps.ags.MapOverlay.prototype.getMapService = function() {
  return this.mapService_;
};
 ...
 */
gmaps.ags.MapOverlay.prototype.refresh = function() {
 ...
 * @return {String}
 //TODO
 gmaps.ags.MapOverlay.prototype.getCopyright  = function (bounds, zoom) {
 ...
 */
gmaps.ags.MapOverlay.prototype.isHidden = function() {
 ...
 * @return {Boolean}
 */
gmaps.ags.MapOverlay.prototype.isInZoomRange_ = function() {
 ...
 */
gmaps.ags.MapOverlay.prototype.show = function() {
  this.visible_ = true;
 ...
 * Hide the overlay
 */
gmaps.ags.MapOverlay.prototype.hide = function() {
  this.visible_ = false;
