  ...
  */ 
  public class SpatialReferences
  {
    ...
    public static function addSpatialReference(wkid:int, wktOrSR:Object):SpatialReference
    {
      ...
      var prj:String = Util.extractString(wkt, "PROJECTION[\"", "\"]");
      var spheroid:Array = Util.extractString(wkt, "SPHEROID[", "]").split(",");
      if (prj !== "") {
        params.unit = parseFloat(Util.extractString(Util.extractString(wkt, "PROJECTION", ""), "UNIT[", "]").split(",")[1]);
        params.semi_major = parseFloat(spheroid[1]);
        params.inverse_flattening = parseFloat(spheroid[2]);
        params.latitude_of_origin = parseFloat(Util.extractString(wkt, "\"Latitude_Of_Origin\",", "]"));
        params.central_meridian = parseFloat(Util.extractString(wkt, "\"Central_Meridian\",", "]"));
        params.false_easting = parseFloat(Util.extractString(wkt, "\"False_Easting\",", "]"));
        params.false_northing = parseFloat(Util.extractString(wkt, "\"False_Northing\",", "]"));
      }
      switch (prj) {
      case "":
      ...
      case "Lambert_Conformal_Conic":
        params.standard_parallel_1 = parseFloat(Util.extractString(wkt, "\"Standard_Parallel_1\",", "]"));
        params.standard_parallel_2 = parseFloat(Util.extractString(wkt, "\"Standard_Parallel_2\",", "]"));
        sr = new LambertConformalConic(params);
        break;
      case "Transverse_Mercator":
        params.scale_factor = parseFloat(Util.extractString(wkt, "\"Scale_Factor\",", "]"));
